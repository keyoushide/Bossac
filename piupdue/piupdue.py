...
def GetPortInfo(Port, PortList): 
    ...
    p = subprocess.Popen(["udevadm info -q path -n " + Port], stdout=subprocess.PIPE, shell=True)
    (output, err) = p.communicate()
    
    p = subprocess.Popen(["udevadm info --query=property -p " + output], stdout=subprocess.PIPE, shell=True)
    (output, err) = p.communicate()
